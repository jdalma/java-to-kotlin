...
1. `private val hash`라는 계산 프로퍼티를 `private`해서 `hash()`를 따로 작성해서 접근하는 이유는 뭘까? 219p
   - 클라이언트에게 계산 프로퍼티와 함수라는 의미를 명확하게 전달하기 위해서?
...
1. "`Money`값을 표현하기 위해 사용하는 동반 객체에 선언한 함수들이 정적 함수라는 점도 코틀린 규약을 깬다" 235p
2. `operator fun invoke()`의 용도, 사용 시점
